...
| 3 | **Stop growing spine / flat stays default** (`decision-trace-family.md`) | Most exposed. The "+14% bulk step" compared LSM-with-in-step-compaction against a dictionary; Feldera merges on W background threads. The memory leg is also weakened (§4: every sibling-less operator has an equivalent, and `STANDARD` views may need no integral). | Move our compaction off the step thread, then re-run the flat-vs-spine bulk step. That single change makes our measurement the experiment we thought we were running. |
| 4 | **We are allocation- and hash-bound; columnar deferred three times** (`design-row-representation.md`, `repr-execution-apportionment.md`) | Reframed, not refuted. Both costs are consequences of the hash-indexed Z-set, not of DBSP. "Flat beats spine" and "we are allocation-bound" may be the same observation seen twice. | Whether our spine is slow *as a sorted-batch design* or slow *as our implementation of one*. Item 3's experiment is the first half of this answer. |
