...
| `PartitionedOffsetOp` | `PartitionedOffsetOp.cs` | one sorted integrated input per `PARTITION BY` key, plus the last-emitted widened output per partition | Incremental `LAG` / `LEAD(expr [, offset [, default]]) OVER (PARTITION BY p ORDER BY o)` — positional offset functions emitted as a new column. Each partition is sorted by a total order (ORDER BY key then full-row tiebreak) and expanded into weight-aware positions; each row reads each spec's value from the row `offset` positions before (LAG) / after (LEAD), or its `default`. Recompute-and-diff the touched partition. The SQL resolver shares the window-function recognition (`WindowOffsetPlan`); structural compile only; no spine variant; no GC yet. |
