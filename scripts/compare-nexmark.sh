...
#                              [--queries="q0 q1 ..."] [--feldera=PATH] [--dbsp-only]
#
# Env overrides (same names, upper-case): EVENTS, CORES, BATCH, RUNS, QUERIES, FELDERA_DIR
#
# Notes:
#   * The first Feldera run compiles the DBSP workspace in release — that can take
...
echo "   events=$EVENTS  cores=$CORES  batch=$BATCH  runs=$RUNS"
...
if not have_feld:
    ...
    print(f"\nDbspNet W={cores} ÷ Feldera at {cores} cores; >1.0x means DbspNet is faster on that query.")
PY
